...
class zpt_orm_test_mock_ManyToManyEntity {
  ...
  */
  public function __construct() {
    ...
    $this->createSql =
      "INSERT INTO many_to_many_entity
      (`name`) VALUES (:name)";
    $this->_create = $this->_pdo->prepare($this->createSql);

    $this->updateSql =
      "UPDATE many_to_many_entity
      SET `name` = :name
      WHERE id = :id";
    $this->_update = $this->_pdo->prepare($this->updateSql);

    $this->deleteSql = "DELETE FROM many_to_many_entity WHERE id = :id";
  ...
  */
  public function count(Criteria $c) {
    ...
    if ($c->getTable() === null) {
      $c->setTable('many_to_many_entity');
    }
    ...
    try {
      ...
      $e = new PdoExceptionWrapper($e, 'zpt\orm\test\mock\ManyToManyEntity');
      $e->setSql($sql, $params);
      throw $e;
    }
  }
  ...
  * @param ${class} $model
  */
  public function create(\zpt\orm\test\mock\ManyToManyEntity $model) {
    $validator = ActorFactory::getActor('validator', 'zpt\orm\test\mock\ManyToManyEntity');
    ...
    try {
      ...
      $transformer = ActorFactory::getActor('transformer', 'zpt\orm\test\mock\ManyToManyEntity');
      ...
      $sql = "DELETE FROM many_to_many_entity_simple_entity_link
      ...
      if ($related !== null) {
        $sql = "INSERT INTO many_to_many_entity_simple_entity_link
                (many_to_many_entity_id, simple_entity_id)
                VALUES (:lhsId, :rhsId)";
  ...
  * @param ${class} $model
  */
  public function delete(\zpt\orm\test\mock\ManyToManyEntity $model) {
    $id = $model->getid();
    if ($id === null) {
      throw new Exception("Can't delete zpt\\orm\\test\\mock\\ManyToManyEntity because it does not have an id");
    }
  ...
  *
  * @param integer $id
  * @return zpt\orm\test\mock\ManyToManyEntity
  */
  ...
  */
  public function getQueryBuilder() {
    return QueryBuilder::get('zpt\orm\test\mock\ManyToManyEntity');
  }
  ...
  public function retrieve(Criteria $c = null) {
    ...
    try {
      ...
      foreach ($stmt AS $row) {
        ...
        // -------------------------------------------------------------------
        // Populate the zpt\orm\test\mock\SimpleEntity
        $c = new Criteria();
        $c->addSelect('simple_entity.*');
        $c->addInnerJoin('many_to_many_entity_simple_entity_link', 'id', 'simple_entity_id');
        $c->addEquals('many_to_many_entity_id', $id);
  ...
  * @param ${class} $model
  */
  public function save(\zpt\orm\test\mock\ManyToManyEntity $model) {
    $id = $model->getid();
    if ($id === null) {
      $this->create($model);
    } else {
  ...
  * @param ${class} $model
  */
  public function update(\zpt\orm\test\mock\ManyToManyEntity $model) {
    $id = $model->getid();
    if ($id === null) {
      throw new Exception("Can't update zpt\\orm\\test\\mock\\ManyToManyEntity because it does not have an id");
    }
    ...
    try {
      ...
      // Delete all link entries for this entity
      $sql = "DELETE FROM many_to_many_entity_simple_entity_link WHERE many_to_many_entity_id = :id";
      $params = array('id' => $id);
      $deleteStmt = $this->_pdo->prepare($sql);
      $deleteStmt->execute($params);
      ...
      // Create new link entries for all related entities
      if ($related !== null) {
        $sql = "INSERT INTO many_to_many_entity_simple_entity_link (many_to_many_entity_id, simple_entity_id) VALUES (:lhsId, :rhsId)";
        $createStmt = $this->_pdo->prepare($sql);
        foreach ($related AS $rel) {
          $params = array(
            'lhsId' => $id,
