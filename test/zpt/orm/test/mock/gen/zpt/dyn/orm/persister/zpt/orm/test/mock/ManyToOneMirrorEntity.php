...
class zpt_orm_test_mock_ManyToOneMirrorEntity {
  ...
  */
  ...
  /**
  * Create a new persister for zpt\orm\test\mock\ManyToOneMirrorEntity entities.
  */
  public function __construct() {
    $this->_pdo = PdoWrapper::get();

    $this->createSql =
      "INSERT INTO many_to_one_mirror
      (`name`,`one_to_many_mirror_id`)
      VALUES
      (:name,:one_to_many_mirror_id)";
    $this->_create = $this->_pdo->prepare($this->createSql);

    $this->updateSql =
      "UPDATE many_to_one_mirror
      SET `name` = :name,`one_to_many_mirror_id` = :one_to_many_mirror_id
      WHERE id = :id";
    $this->_update = $this->_pdo->prepare($this->updateSql);

    $this->deleteSql = "DELETE FROM many_to_one_mirror WHERE id = :id";
  ...
  */
  public function count(Criteria $c) {
    ...
    try {
      ...
      $e = new PdoExceptionWrapper($e, 'zpt\orm\test\mock\ManyToOneMirrorEntity');
      $e->setSql($sql, $params);
      throw $e;
    }
  }
  ...
  *
  * @param zpt\orm\test\mock\ManyToOneMirrorEntity $model
  */
  public function create(\zpt\orm\test\mock\ManyToOneMirrorEntity $model) {
    $validator = ActorFactory::getActor('validator', 'zpt\orm\test\mock\ManyToOneMirrorEntity');
    ...
    try {
      ...
      // Populate zpt\orm\test\mock\OneToManyMirrorEntity parameter
      $rhs = $model->getone();
      $rhsId = null;
      if ($rhs !== null) {
        $rhsId = $rhs->getid();
      ...
      $params[':one_to_many_mirror_id'] = $rhsId;

      $sql = $this->createSql; // If there is an exception this is handy to know
      $this->_create->execute($params);

      $transformer = ActorFactory::getActor('transformer', 'zpt\orm\test\mock\ManyToOneMirrorEntity');
      ...
      // ---------------------------------------------------------------------
      // Save related zpt\orm\test\mock\OneToManyMirrorEntity entities
      $persister = Persister::get('zpt\orm\test\mock\OneToManyMirrorEntity');
      // ---------------------------------------------------------------------
  ...
  * @param zpt\orm\test\mock\ManyToOneMirrorEntity $model
  */
  public function delete(\zpt\orm\test\mock\ManyToOneMirrorEntity $model) {
    $id = $model->getid();
    if ($id === null) {
      throw new Exception("Can't delete zpt\\orm\\test\\mock\\ManyToOneMirrorEntity because it does not have an id");
    }
  ...
  *
  * @param integer $id
  * @return zpt\orm\test\mock\ManyToOneMirrorEntity
  */
  ...
  */
  public function getQueryBuilder() {
    return QueryBuilder::get('zpt\orm\test\mock\ManyToOneMirrorEntity');
  }
  ...
  public function retrieve(Criteria $c = null, $debug = false) {
    ...
    try {
      ...
      foreach ($stmt AS $row) {
        ...
        // -------------------------------------------------------------------
        // Populate the zpt\orm\test\mock\OneToManyMirrorEntity
        $relId = $row['one_to_many_mirror_id'];
        if ($relId !== null) {
          $persister = Persister::get('zpt\orm\test\mock\OneToManyMirrorEntity');
          $related = $persister->getById($relId);

          if ($related === null) {
            throw new Exception("No zpt\\orm\\test\\mock\\OneToManyMirrorEntity with id $relId.");
          }

          $model->setone($related);
        }
  ...
  * @param zpt\orm\test\mock\ManyToOneMirrorEntity $model
  */
  public function save(\zpt\orm\test\mock\ManyToOneMirrorEntity $model) {
    $id = $model->getid();
    if ($id === null) {
      $this->create($model);
    } else {
  ...
  * @param zpt\orm\test\mock\ManyToOneMirrorEntity $model
  */
  public function update(\zpt\orm\test\mock\ManyToOneMirrorEntity $model, $debug = false) {
    $id = $model->getid();
    if ($id === null) {
      throw new Exception("Can't update zpt\\orm\\test\\mock\\ManyToOneMirrorEntity because it does not have an id");
    }
    ...
    try {
      ...
      if ($rhs !== null) {
        $rhsId = $rhs->getid();
        if ($rhsId === null) {
          $persister = ActorFactory::getActor('persister', 'zpt\orm\test\mock\OneToManyMirrorEntity');
          $rhsId = $persister->create($rhs);
        }
      }
      $params[':one_to_many_mirror_id'] = $rhsId;
