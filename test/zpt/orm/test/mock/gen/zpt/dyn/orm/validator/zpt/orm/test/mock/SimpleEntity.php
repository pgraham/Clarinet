...
class zpt_orm_test_mock_SimpleEntity {
  ...
  public function validate(\zpt\orm\test\mock\SimpleEntity $model, &$e = false) {
    ...
    $msg = $this->_checkvalue($model->getvalue());
    if ($msg !== null) {
      $msgs[] = $msg;
    }

    if (count($msgs) > 0) {
      $ex = new ValidationException($msgs, 'zpt\orm\test\mock\SimpleEntity');
      if ($e !== false) {
        $e = $ex;
      } else {
        throw $ex;
  ...
  private function _checkvalue($val) {
    if ($val === null) {
      return null;
    }
