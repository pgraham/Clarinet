...
*/
class zpt_orm_test_mock_ManyToManyEntity {

  /**
  * Transform the given model object into an array.
  *
  * @param zpt\orm\test\mock\ManyToManyEntity $model The model instance to convert.
  */
  public function asArray(\zpt\orm\test\mock\ManyToManyEntity $model = null) {
    if ($model === null) {
      return null;
    }
    ...
    if ($relVal === null) {
      ...
      $transformer = Transformer::get('zpt\orm\test\mock\SimpleEntity');
      $rels = array();
  ...
  /**
  * Transform an array of zpt\orm\test\mock\ManyToManyEntity instances into an array.
  *
  * @param zpt\orm\test\mock\ManyToManyEntity[] $models
  */
  public function asCollection(array $models) {
    $a = array();
    foreach ($models AS $model) {
      if (!($model instanceof \zpt\orm\test\mock\ManyToManyEntity)) {
  ...
  */
  public function fromArray(array $a, $model = null, array $whiteList = null) {
    if ($model === null) {
      if (isset($a['id'])) {
        $persister = Persister::get('zpt\orm\test\mock\ManyToManyEntity');
        $model = $persister->getById($a['id']);
      } else {
        $model = new \zpt\orm\test\mock\ManyToManyEntity();
      }
    }
