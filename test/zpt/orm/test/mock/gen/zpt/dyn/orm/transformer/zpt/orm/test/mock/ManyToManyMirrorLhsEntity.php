...
*/
class zpt_orm_test_mock_ManyToManyMirrorLhsEntity {

  /**
  * Transform the given model object into an array.
  *
  * @param zpt\orm\test\mock\ManyToManyMirrorLhsEntity $model The model instance to convert.
  */
  public function asArray(\zpt\orm\test\mock\ManyToManyMirrorLhsEntity $model = null) {
    if ($model === null) {
      return null;
    }
    ...
    if ($relVal === null) {
      ...
      $transformer = Transformer::get('zpt\orm\test\mock\ManyToManyMirrorRhsEntity');
      $rels = array();
  ...
  /**
  * Transform an array of zpt\orm\test\mock\ManyToManyMirrorLhsEntity instances into an array.
  *
  * @param zpt\orm\test\mock\ManyToManyMirrorLhsEntity[] $models
  */
  public function asCollection(array $models) {
    $a = array();
    foreach ($models AS $model) {
      if (!($model instanceof \zpt\orm\test\mock\ManyToManyMirrorLhsEntity)) {
  ...
  */
  public function fromArray(array $a, $model = null, array $whiteList = null) {
    if ($model === null) {
      if (isset($a['id'])) {
        $persister = Persister::get('zpt\orm\test\mock\ManyToManyMirrorLhsEntity');
        $model = $persister->getById($a['id']);
      } else {
        $model = new \zpt\orm\test\mock\ManyToManyMirrorLhsEntity();
      }
    }
