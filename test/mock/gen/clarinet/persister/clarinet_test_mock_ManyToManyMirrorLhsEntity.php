...
use \clarinet\validator\clarinet_test_mock_ManyToManyMirrorLhsEntity as Validator;
...
class clarinet_test_mock_ManyToManyMirrorLhsEntity {
  ...
  public function __construct(PDO $pdo) {
    ...
    $this->_create = $this->_pdo->prepare(
      "INSERT INTO many_to_many_lhs_entity (name) VALUES (:name)");

    $this->_update = $this->_pdo->prepare(
      "UPDATE many_to_many_lhs_entity SET name = :name WHERE id = :id");

    $this->_delete = $this->_pdo->prepare(
      "DELETE FROM many_to_many_lhs_entity WHERE id = :id");
  }
  ...
  public function getById($id) {
    if (!isset($this->_cache[$id])) {
      ...
      if (!isset($this->_cache[$id])) {
        throw new Exception("No clarinet\test\mock\ManyToManyMirrorLhsEntity exists with id $id");
      }
    }
    return $this->_cache[$id];
  }

  public function create(\clarinet\test\mock\ManyToManyMirrorLhsEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error creating clarinet\test\mock\ManyToManyMirrorLhsEntity: ' . $e->getMessage(), $e);
    }
  }

  public function retrieve(Criteria $c = null) {
    if ($c->getTable() === null) {
      $c->setTable('many_to_many_lhs_entity');
    }
...
// -------------------------------------------------------------------
// Populate the clarinet\test\mock\ManyToManyMirrorRhsEntity
$c = new Criteria();
$c->addJoin('many_to_many_lhs_entity_many_to_many_rhs_entity_link', 'id', 'many_to_many_rhs_entity_id');
$c->addEquals('many_to_many_lhs_entity_id', $id);

$persister = AbstractActorFactory::getActor('persister', 'clarinet\test\mock\ManyToManyMirrorRhsEntity');
...
// -------------------------------------------------------------------
      ...
      throw new Exception('Error retrieving clarinet\test\mock\ManyToManyMirrorLhsEntity instances', $e);
    }
  }

  public function update(\clarinet\test\mock\ManyToManyMirrorLhsEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error updating clarinet\test\mock\ManyToManyMirrorLhsEntity', $e);
    }
  }

  public function delete(\clarinet\test\mock\ManyToManyMirrorLhsEntity $model) {
    $params = Array();
    $params[':id'] = $model->getId();
