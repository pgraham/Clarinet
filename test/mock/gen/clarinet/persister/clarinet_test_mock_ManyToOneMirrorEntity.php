...
use \clarinet\validator\clarinet_test_mock_ManyToOneMirrorEntity as Validator;
...
class clarinet_test_mock_ManyToOneMirrorEntity {
  ...
  public function __construct(PDO $pdo) {
    ...
    $this->_create = $this->_pdo->prepare(
      "INSERT INTO many_to_one_mirror (name) VALUES (:name)");

    $this->_update = $this->_pdo->prepare(
      "UPDATE many_to_one_mirror SET name = :name WHERE id = :id");

    $this->_delete = $this->_pdo->prepare(
      "DELETE FROM many_to_one_mirror WHERE id = :id");
  }
  ...
  public function getById($id) {
    if (!isset($this->_cache[$id])) {
      ...
      if (!isset($this->_cache[$id])) {
        throw new Exception("No clarinet\test\mock\ManyToOneMirrorEntity exists with id $id");
      }
    }
    return $this->_cache[$id];
  }

  public function create(\clarinet\test\mock\ManyToOneMirrorEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error creating clarinet\test\mock\ManyToOneMirrorEntity: ' . $e->getMessage(), $e);
    }
  }
...
// -------------------------------------------------------------------
// Populate the clarinet\test\mock\OneToManyMirrorEntity
$relId = $row['one_to_many_mirror_id'];
if ($relId !== null) {
  $persister = ActorAbstractFactory::getActor('persister', 'clarinet\test\mock\OneToManyMirrorEntity');
  $related = $persister->getById($relId);

  if ($related === null) {
    throw new Exception("No clarinet\test\mock\OneToManyMirrorEntity with id $relId.");
  }
...
// -------------------------------------------------------------------
      ...
      throw new Exception('Error retrieving clarinet\test\mock\ManyToOneMirrorEntity instances', $e);
    }
  }

  public function update(\clarinet\test\mock\ManyToOneMirrorEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error updating clarinet\test\mock\ManyToOneMirrorEntity', $e);
    }
  }

  public function delete(\clarinet\test\mock\ManyToOneMirrorEntity $model) {
    $params = Array();
    $params[':id'] = $model->getId();
