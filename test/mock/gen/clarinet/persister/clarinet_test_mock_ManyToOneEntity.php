...
use \clarinet\validator\clarinet_test_mock_ManyToOneEntity as Validator;
...
class clarinet_test_mock_ManyToOneEntity {
  ...
  public function __construct(PDO $pdo) {
    ...
    $this->_create = $this->_pdo->prepare(
      "INSERT INTO many_to_one_entity (name) VALUES (:name)");

    $this->_update = $this->_pdo->prepare(
      "UPDATE many_to_one_entity SET name = :name WHERE id = :id");

    $this->_delete = $this->_pdo->prepare(
      "DELETE FROM many_to_one_entity WHERE id = :id");
  }
  ...
  public function getById($id) {
    if (!isset($this->_cache[$id])) {
      ...
      if (!isset($this->_cache[$id])) {
        throw new Exception("No clarinet\test\mock\ManyToOneEntity exists with id $id");
      }
    }
    return $this->_cache[$id];
  }

  public function create(\clarinet\test\mock\ManyToOneEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error creating clarinet\test\mock\ManyToOneEntity: ' . $e->getMessage(), $e);
    }
  }

  public function retrieve(Criteria $c = null) {
    if ($c->getTable() === null) {
      $c->setTable('many_to_one_entity');
    }
...
// -------------------------------------------------------------------
// Populate the clarinet\test\mock\SimpleEntity
$relId = $row['simple_entity_id'];
if ($relId !== null) {
  $persister = ActorAbstractFactory::getActor('persister', 'clarinet\test\mock\SimpleEntity');
  $related = $persister->getById($relId);

  if ($related === null) {
    throw new Exception("No clarinet\test\mock\SimpleEntity with id $relId.");
  }
...
// -------------------------------------------------------------------
      ...
      throw new Exception('Error retrieving clarinet\test\mock\ManyToOneEntity instances', $e);
    }
  }

  public function update(\clarinet\test\mock\ManyToOneEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error updating clarinet\test\mock\ManyToOneEntity', $e);
    }
  }

  public function delete(\clarinet\test\mock\ManyToOneEntity $model) {
    $params = Array();
    $params[':id'] = $model->getId();
