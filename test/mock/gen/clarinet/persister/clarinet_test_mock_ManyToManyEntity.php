...
use \clarinet\validator\clarinet_test_mock_ManyToManyEntity as Validator;
...
class clarinet_test_mock_ManyToManyEntity {
  ...
  public function __construct(PDO $pdo) {
    ...
    $this->_create = $this->_pdo->prepare(
      "INSERT INTO many_to_many_entity (name) VALUES (:name)");

    $this->_update = $this->_pdo->prepare(
      "UPDATE many_to_many_entity SET name = :name WHERE id = :id");

    $this->_delete = $this->_pdo->prepare(
      "DELETE FROM many_to_many_entity WHERE id = :id");
  }
  ...
  public function getById($id) {
    if (!isset($this->_cache[$id])) {
      ...
      if (!isset($this->_cache[$id])) {
        throw new Exception("No clarinet\test\mock\ManyToManyEntity exists with id $id");
      }
    }
    return $this->_cache[$id];
  }

  public function create(\clarinet\test\mock\ManyToManyEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error creating clarinet\test\mock\ManyToManyEntity: ' . $e->getMessage(), $e);
    }
  }
...
$c = new Criteria();
$c->addJoin('many_to_many_entity_simple_entity_link', 'id', 'simple_entity_id');
$c->addEquals('many_to_many_entity_id', $id);

$persister = AbstractActorFactory::getActor('persister', 'clarinet\test\mock\SimpleEntity');
...
// -------------------------------------------------------------------
      ...
      throw new Exception('Error retrieving clarinet\test\mock\ManyToManyEntity instances', $e);
    }
  }

  public function update(\clarinet\test\mock\ManyToManyEntity $model) {
    ...
    try {
      ...
      throw new Exception('Error updating clarinet\test\mock\ManyToManyEntity', $e);
    }
  }

  public function delete(\clarinet\test\mock\ManyToManyEntity $model) {
    $params = Array();
    $params[':id'] = $model->getId();
