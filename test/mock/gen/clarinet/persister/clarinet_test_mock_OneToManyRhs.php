...
use \clarinet\validator\clarinet_test_mock_OneToManyRhs as Validator;
...
class clarinet_test_mock_OneToManyRhs {
  ...
  public function __construct(PDO $pdo) {
    ...
    $this->_create = $this->_pdo->prepare(
      "INSERT INTO one_to_many_rhs (name) VALUES (:name)");

    $this->_update = $this->_pdo->prepare(
      "UPDATE one_to_many_rhs SET name = :name WHERE id = :id");

    $this->_delete = $this->_pdo->prepare(
      "DELETE FROM one_to_many_rhs WHERE id = :id");
  }
  ...
  public function getById($id) {
    if (!isset($this->_cache[$id])) {
      ...
      if (!isset($this->_cache[$id])) {
        throw new Exception("No clarinet\test\mock\OneToManyRhs exists with id $id");
      }
    }
    return $this->_cache[$id];
  }

  public function create(\clarinet\test\mock\OneToManyRhs $model) {
    ...
    try {
      ...
      throw new Exception('Error creating clarinet\test\mock\OneToManyRhs: ' . $e->getMessage(), $e);
    }
  }
  ...
  public function retrieve(Criteria $c = null) {
    ...
    try {
      ...
      throw new Exception('Error retrieving clarinet\test\mock\OneToManyRhs instances', $e);
    }
  }

  public function update(\clarinet\test\mock\OneToManyRhs $model) {
    ...
    try {
      ...
      throw new Exception('Error updating clarinet\test\mock\OneToManyRhs', $e);
    }
  }

  public function delete(\clarinet\test\mock\OneToManyRhs $model) {
    $params = Array();
    $params[':id'] = $model->getId();
